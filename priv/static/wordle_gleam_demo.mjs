...
// build/dev/javascript/wordle_gleam_demo/app.ffi.mjs
async function delay(amount, cb) {
...
// build/dev/javascript/wordle_gleam_demo/shared.mjs
var Correct = class extends CustomType {
};
var WrongPos = class extends CustomType {
};
var Wrong = class extends CustomType {
};
...
var GridLetter = class extends CustomType {
...
var Model = class extends CustomType {
  constructor(words, word, guess, guesses, used_letters, error, game_state, end_game_modal2) {
    super();
    this.words = words;
    this.word = word;
    this.guess = guess;
    ...
    this.end_game_modal = end_game_modal2;
  }
};
var ApiReturnedWords = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserSentGameInput = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var ErrorDelayFinished = class extends CustomType {
};
var UserChangedModalState = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
var too_short_err = "Too short";

// build/dev/javascript/wordle_gleam_demo/components/end_game_modal.mjs
function update_end_game_modal(model, msg) {
  if (msg instanceof UserChangedModalState) {
    let state = msg[0];
    if (state instanceof Closed) {
      ...
      return new Ok(
        [model.withFields({ end_game_modal: state }), none()]
      );
    }
  } else {
    return new Error(void 0);
  ...
  if ($) {
    return div(
      append(
        ...
        (() => {
          let $1 = model.end_game_modal;
          if ($1 instanceof Open) {
            return toList([class$("ease-in")]);
...
function add_new_guess(guesses, guess, word) {
  ...
  let _pipe = map22(
    ...
    word_letters,
    (g, w) => {
      let $ = contains_string(word, g);
      if ($ && g === w) {
        let g$1 = g;
        let w$1 = w;
        return new GridLetter(g$1, new Correct());
      } else if ($) {
        return new GridLetter(g, new WrongPos());
      } else {
        return new GridLetter(g, new Wrong());
      }
    }
  );
  let _pipe$1 = ((g) => {
...
function handle_guess(model) {
  ...
  let $1 = contains(model.words, model.guess);
  if ($1 && $ === 5) {
    let g = $;
    let state = (() => {
      let $2 = model.guess === model.word;
      let $3 = length(model.guesses) + 1;
      if ($2) {
        return new Won();
      } else if (!$2 && $3 === 5) {
        let tries = $3;
    ...
    let g = $;
    return model.withFields({ error: not_word_err });
  } else if ($ < 5) {
    let g = $;
    return model.withFields({ error: too_short_err });
  } else {
    return model;
  }
}
function get_alpha(input) {
  ...
  let re = $[0];
  let letter = lowercase2(input);
  let $1 = check(re, letter);
  if ($1) {
    return letter;
  } else {
    return "";
...
function handle_input(model, input) {
  let model$1 = model.withFields({ error: "" });
  let $ = length3(input);
  let $1 = length3(model$1.guess);
  if ($ > 1) {
    let i = $;
    if (input === "Backspace") {
      return model$1.withFields({ guess: drop_right(model$1.guess, 1) });
    } else if (input === "Enter") {
      return handle_guess(model$1);
    } else {
      return model$1;
    }
  } else if ($1 < 5) {
    let g = $1;
    return model$1.withFields({ guess: model$1.guess + get_alpha(input) });
  } else {
    return model$1;
  }
}
function update_grid(model, msg) {
  if (msg instanceof UserSentGameInput) {
    let input = msg[0];
    let $ = model.game_state;
    if ($ instanceof Playing) {
      let new_model = handle_input(model, input);
      let input_effect = (() => {
        let $1 = new_model.error !== "" && model.error === "";
        if ($1) {
          return delay2(1500, new ErrorDelayFinished());
        } else {
          return none();
        }
      })();
      return new Ok([new_model, input_effect]);
    } else {
...
function word_grid(model) {
  let grid_letters = concat2(
    toList([
      ...
      (() => {
        ...
        if ($ instanceof Playing) {
          return toList([
            append(
              map2(
                graphemes(model.guess),
                (l) => {
                  return new GridLetter(l, new Pending());
                }
              ),
              repeat(
                new GridLetter("", new Pending()),
                guess_max - length3(model.guess)
              )
            )
          ]);
        } else {
      ...
      repeat(
        repeat(new GridLetter("", new Pending()), guess_max),
        guess_max - (() => {
          let $ = model.game_state;
          if ($ instanceof Playing) {
            return length(model.guesses) + 1;
          } else {
            return length(model.guesses);
          }
        })()
      )
    ])
  ...
  return div(
    ...
    (() => {
      let _pipe = map2(
        ...
        (row) => {
          return div(
            toList([class$("grid_row")]),
            map2(
              row,
              (g_letter) => {
                return div(
                  append(
                    toList([class$("grid_letter_container")]),
                    (() => {
                      let $ = g_letter.status;
                  ...
                  toList([text(uppercase2(g_letter.letter))])
                );
              }
            )
          );
...
function update_api(model, msg) {
  if (msg instanceof ApiReturnedWords && msg[0].isOk()) {
    ...
    return new Ok(
      [
        model.withFields({ words: response, word: get_random_word(response) }),
        none()
      ]
    );
  } else if (msg instanceof ApiReturnedWords && !msg[0].isOk()) {
    return new Ok([model, none()]);
  } else if (msg instanceof ErrorDelayFinished) {
    return new Ok([model.withFields({ error: "" }), none()]);
  } else {
    return new Error(void 0);
  }
}
function update2(model, msg) {
  let _pipe = find_map(
    toList([update_api, update_grid, update_end_game_modal]),
    (update_func) => {
      return update_func(model, msg);
    }
  );
  return unwrap2(_pipe, [model, none()]);
}
function get_words() {
  let decoder = list(string);
  let expect = expect_json(
    decoder,
    (var0) => {
      return new ApiReturnedWords(var0);
    }
  );
  return get2(
    "http://localhost:1234/priv/static/words.json",
    expect
  );
}
function init2(_) {
